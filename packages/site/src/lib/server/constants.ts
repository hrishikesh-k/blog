...
export class HBlob {
  ...
  constructor(store_name : string) {
    this.#api = wretch(`https://api.netlify.com/api/v1/blobs/${SITE_ID}/${store_name}`).addon(wretchQueryStringAddon).auth(`Bearer ${NETLIFY_API_KEY}`)
  }
...
export const notion = wretch('https://api.notion.com/v1').auth(`Bearer ${NOTION_API_KEY}`).headers({
