import {all_posts_cache_file, cache_dir, notion} from '~/lib/server/constants.ts'
...
export async function load_all_posts(logger : HLogger) {
  ...
  if (existsSync(all_posts_cache_file)) {
    try {
      ...
      const all_posts : Array<{
        ...
        title : string
        updated_at : string
      }> = JSON.parse(readFileSync(all_posts_cache_file, {
        encoding: 'utf-8'
      }))
  ...
  let all_posts_from_notion : TNRes<'page_or_database', Pick<TNPage, 'cover' | 'icon' | 'object' | 'parent' | 'properties' | 'public_url' | 'url'>>

  try {
    logger.info('fetching entries')
    all_posts_from_notion = await notion.post(null, '/databases/ee93ad0cead84102868aae5665d8d2d1/query').json()
    logger.success(`fetched ${all_posts_from_notion.results.length} total entries`)
  } catch (e) {
    logger.error('failed to fetch entries')
    throw e
  }

  const all_posts_filtered = all_posts_from_notion.results.filter(p => !p.archived && !p.in_trash && p.properties.Status.status.name === 'Published').map(p => ({
    id: p.id,
